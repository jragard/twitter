    ...
    if logged_in_user is not None:
        ...
        for x in results:
            if (x.twitter_user in username_following_list
                    or x.twitter_user == logged_in_user):
...
def user_profile_view(request, user):
    ...
    if request.method == 'POST':
        targeted_user = (TwitterUser.objects.filter(
...
def compose_view(request):
    ...
    if form.is_valid():
        ...
        Tweet.objects.create(
            ...
            date_time=(x.strftime("%b") + " " + str(x.day) + ", " +
                       str(x.year) + ", " + str(x.strftime("%I")) +
                       ":" + str(x.strftime("%M")) + " " + x.strftime("%p")),
            viewed=False
        )
